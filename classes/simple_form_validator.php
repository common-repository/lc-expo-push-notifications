...
class simple_fv {
	...
	private function getIndexVal($index) {
		...
		elseif(isset($_FILES[$index])) {
            $index_val = $index;
        } // return index - will get the val during validation
		else {
            $index_val = false;
        }
		
		if(!is_array($index_val)) {
            return sanitize_textarea_field(trim((string)$index_val));
        }
		else {
            return array_map('sanitize_textarea_field', $index_val);
        }
	}
